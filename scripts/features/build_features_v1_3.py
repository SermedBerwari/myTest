...
def per_90(
    ...
    """Convert a historical metric to a per-90 rate safely."""
    if value is None or minutes is None or minutes <= 0:
        return None
    return safe_ratio(float(value), float(minutes) / 90.0)


def add_player_involvement_features(
    row: dict[str, Any],
) -> None:
    """
    Add player involvement/per-90 features using only already-created
    pre-target-GW rolling features.

    This is deliberately isolated from the existing v1.0 calculations.
    """
    for window in WINDOWS:
        minutes = row.get(f"last_{window}_minutes")

        row[f"last_{window}_points_per_90_v11"] = per_90(
            row.get(f"last_{window}_total_points"),
            minutes,
        )
        row[f"last_{window}_goals_per_90_v11"] = per_90(
            row.get(f"last_{window}_goals_scored"),
            minutes,
        )
        row[f"last_{window}_assists_per_90_v11"] = per_90(
            row.get(f"last_{window}_assists"),
            minutes,
        )
        row[f"last_{window}_bonus_per_90_v11"] = per_90(
            row.get(f"last_{window}_bonus"),
            minutes,
        )
        row[f"last_{window}_bps_per_90_v11"] = per_90(
            row.get(f"last_{window}_bps"),
            minutes,
        )
        row[f"last_{window}_xg_per_90_v11"] = per_90(
            row.get(f"last_{window}_expected_goals"),
            minutes,
        )
        row[f"last_{window}_xa_per_90_v11"] = per_90(
            row.get(f"last_{window}_expected_assists"),
            minutes,
        )
        row[f"last_{window}_xgi_per_90_v11"] = per_90(
            row.get(f"last_{window}_expected_goal_involvements"),
            minutes,
        )
...
def build_player_feature_row(
    ...
    # ------------------------------------------------------------------
    # These fields are explicit audit metadata, not predictive features.
    row["feature_cutoff_gw"] = target_gameweek - 1
    row["target_gw"] = target_gameweek

    # Player involvement/per-90 features are derived exclusively from
    # the existing v1.0 pre-target rolling values.
    add_player_involvement_features(row)
...
def classify_output_columns(rows: list[dict[str, Any]]) -> tuple[list[str], list[str], list[str], list[str]]:
    """Separate model features, audit metadata, and targets."""
    if not rows:
        return [], [], [], []
    preferred = ["player_id","season","gameweek","web_name","position_id","position_name",
                 "team_id","opponent_team_id","was_home","fixture_difficulty",
                 "prior_gameweeks","prior_minutes"]
    targets = ["target_minutes","target_points","target_goals","target_assists",
               "target_clean_sheets","target_bonus","target_xg","target_xa"]
    audits = ["feature_cutoff_gw","target_gw"]
    all_fields = list(rows[0].keys())
    features = [f for f in preferred if f in all_fields]
    features += [f for f in all_fields if f not in features and f not in targets and f not in audits]
    missing = [f for f in targets + audits if f not in all_fields]
    if missing:
        fail("Required target/audit columns missing: " + ", ".join(missing))
    return features + audits + targets, features, targets, audits
...
def main() -> int:
    ...
    fields, feature_columns, target_columns, audit_columns = classify_output_columns(output_rows)
    ...
    if output_rows:
        ...
        leaked_names = [
            ...
            if f.startswith("target_") or f in {"target_gw", "feature_cutoff_gw"}
        ]
        ...
        invalid_cutoffs = [
            ...
            if r.get("feature_cutoff_gw") is None
            or r.get("target_gw") is None
            or int(r["feature_cutoff_gw"]) >= int(r["target_gw"])
        ]
        if invalid_cutoffs:
            fail(
                "Leakage audit failed: feature_cutoff_gw must be "
                "strictly less than target_gw."
            )
        ...
        missing_v11 = [
            field for field in required_v11
            if field not in fields
        ]
        if missing_v11:
            fail(
                "v1.1 required feature columns missing: "
                + ", ".join(missing_v11)
            )
        forbidden = {"target_gw", "feature_cutoff_gw"} & set(feature_columns)
        if forbidden:
            fail("Audit metadata incorrectly classified as model features: " + ", ".join(sorted(forbidden)))
    ...
    report = {
        ...
        "output_counts": {
            ...
            "audit_columns": len(audit_columns),
            "target_columns": len(target_columns),
            "total_output_columns": len(fields),
        ...
        "leakage_policy": {
            ...
            "feature_cutoff_field": "feature_cutoff_gw",
            "target_gameweek_field": "target_gw",
        ...
        "feature_groups": {
            ...
            "player_involvement_v1_1": [
                "last_*_points_per_90_v11",
                "last_*_goals_per_90_v11",
                "last_*_assists_per_90_v11",
                "last_*_bonus_per_90_v11",
                "last_*_bps_per_90_v11",
                "last_*_xg_per_90_v11",
                "last_*_xa_per_90_v11",
                "last_*_xgi_per_90_v11",
            ],
            "leakage_audit_v1_3": [
                "feature_cutoff_gw",
                "target_gw",
        ...
        "notes": [
            ...
            "v1.1 additions are isolated from the existing v1.0 feature calculations.",
            "v1.1 player involvement features are derived from pre-target v1.0 rolling metrics.",
            "v1.2 fixes the rolling starts aggregation required by start-rate features.",
            "v1.3 explicitly separates model features, audit metadata, and targets.",
        ],
    }
    ...
    LOG.info("Model feature columns  : %d", len(feature_columns))
    LOG.info("Audit columns          : %d", len(audit_columns))
    LOG.info("Target columns         : %d", len(target_columns))
