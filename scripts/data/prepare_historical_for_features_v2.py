...
def main() -> None:
    ...
    players["position_id"] = pos_series.map(lambda x: POSITION_MAP.get(str(x), (None, None))[0]).values
    players["position_name"] = pos_series.map(lambda x: POSITION_MAP.get(str(x), (None, None))[1]).values
